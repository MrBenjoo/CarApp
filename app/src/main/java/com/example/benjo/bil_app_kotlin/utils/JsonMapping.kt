...
class JsonMapping {
    ...
    fun basicInfoMapping(key: String, value: String?): Row = when (key) {
        ...
        Constants.STATUS -> row(R.string.api_status, ExplanationHandler().statusType(value))
        Constants.COLOR -> row(R.string.api_color, value)
        Constants.TYPE -> row(R.string.api_type, ExplanationHandler().vehType(value))
    ...
    fun techInfoMapping(key: String, value: String?): Row = when (key) {
        Constants.HP_1 -> row(R.string.api_power_hp_1, value)
        Constants.KW_1 -> row(R.string.api_power_kw_1, "$value kW")
        Constants.CYL_VOL -> row(R.string.api_cylinder_volume, value)
        Constants.TOP_SPEED -> row(R.string.api_top_speed, "$value km/h")
        Constants.FUEL_1 -> row(R.string.api_fuel_1, value)
        Constants.CONSUMPTION_1 -> row(R.string.api_consumption_1, "$value liter/100km")
        Constants.CO2_1 -> row(R.string.api_co2_1, "$value g/km")
        Constants.TRANSMISSION -> row(R.string.api_transmission, ExplanationHandler().transType(value))
        Constants.FOUR_WHEEL_DRIVE -> row(R.string.api_four_wheel_drive, ExplanationHandler().boolType(value))
        Constants.SOUND_LVL_1 -> row(R.string.api_sound_level, "$value dB")
        Constants.NO_PASSENGERS -> row(R.string.api_number_of_passengers, value)
        Constants.PASSENGER_AB -> row(R.string.api_passenger_airbag, ExplanationHandler().boolType(value))
        Constants.HITCH -> row(R.string.api_hitch, value)
        Constants.CHASSI_CODE_1 -> row(R.string.api_chassi_code_1, value)
        ...
        Constants.NOX_1 -> row(R.string.api_nox_1, "$value g/km?")
        Constants.THC_NOX_1 -> row(R.string.api_thc_nox_1, "$value g/km?")
        Constants.ECO_CLASS -> row(R.string.api_eco_class, ExplanationHandler().ecoClassType(value))
        Constants.EMISS_CLASS -> row(R.string.api_emission_class, ExplanationHandler().emissionType(value))
